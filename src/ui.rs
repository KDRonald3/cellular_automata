//! Iced-based interactive UI for the simulator.
//!
//! This is the runtime selected by [`OutputKind::Ui`](crate::OutputKind::Ui).
//! The library API passes a [`UiSeed`] that populates the form fields with
//! reasonable defaults; the user can change them and click Run.
//!
//! The UI must be invoked from the main thread on macOS and on Windows
//! depending on the graphics backend, and a single process can only open
//! the iced application once. The library documents these constraints
...
use crate::export;
use crate::sim::{make_initial_row, step_row_into, BoundaryMode, PaddingAlign, PaddingFill, Rule};
...
/// Seed values for the interactive UI's form fields.
///
/// The library API translates a [`SimConfig`](crate::SimConfig),
/// [`InitialRow`](crate::InitialRow), and [`RenderOptions`](crate::RenderOptions)
/// into this struct, but downstream code can also construct it directly.
#[derive(Clone, Debug)]
pub struct UiSeed {
    /// String of `0`/`1` characters used as the seed pattern when the user
    /// presses Run. Length need not equal `width`; padding fills the rest.
    ...
    /// Initial cell size in canvas pixels.
    pub cell_size: u32,
    pub show_borders: bool,
    /// Directory used by the in-UI "Export HTML" button. The thin binary
    /// defaults this to `./runs/`.
    pub export_dir: PathBuf,
}

/// Open the interactive iced window using the given seed values and block
/// until the user closes it.
pub fn run_ui(seed: UiSeed) -> iced::Result {
    iced::application(App::title, App::update, App::view)
        .subscription(App::subscription)
        .theme(|_| Theme::Light)
        .run_with(move || App::new_seeded(seed))
}
...
impl App {
    ...
    fn new_seeded(seed: UiSeed) -> (Self, iced::Task<Message>) {
        (
            App {
                initial_text: seed.initial_text,
                rule_text: seed.rule.to_string(),
                generations_text: seed.generations.to_string(),
                width_text: seed.width.to_string(),
                cell_size_text: seed.cell_size.max(1).to_string(),
                boundary: seed.boundary,
                padding_align: seed.padding_align,
                padding_fill: seed.padding_fill,
                show_borders: seed.show_borders,
                export_dir: seed.export_dir,
                next_job_id: 1,
                jobs: Vec::new(),
                selected: None,
                error: None,
        ...
        match message {
            ...
            Message::ExportJob(id) => {
                if let Some(job) = self.jobs.iter().find(|j| j.id == id) {
                    ...
                    match export::export_job(&input, &self.export_dir) {
                        Ok(path) => {
